...
public class Chess_player(Chess_tools.Turns player_color) : Chess_player_root(player_color)
{
    ...
    {
        if (end_game_type != Chess_tools.Loose_type.game_gos || _board[pos.row, pos.col].color != turn)
            return [];
        ...
        possible_moves.UnionWith(current.get_moves(_board).Where(m => Chess_tools.is_valid_move(_board, m, current.color, move_bools.is_king_moved, draw_data)).Select(m => m.to));
